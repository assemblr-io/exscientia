...
# Read Data & clean up leading and tailing column header white spaces
assay_results = pd.read_csv("./assay_results.csv", sep='\s*,\s*', engine='python')
labels = pd.read_csv("./compound_labels.csv", sep='\s*,\s*', engine='python')
IC50 = pd.read_csv("./compound_ic50.csv", sep='\s*,\s*', engine='python')
...
grouped_data = df_results.groupby(['Compound ID', 'Assay Result Label', 'IC50 (M)'])['% Inhibition'].max().reset_index(
    name='Maximum % Inhibition')
...
app.layout = html.Div([
    ...
    html.Div([
        html.H2("Distribution of Compound by Assay Result",
                style={'textAlign': 'center', 'lineHeight': 1, 'fontSize': '1.3em'}),
    ...
    html.Div([
        html.H2("Compound Activity Details", style={'textAlign': 'center', 'lineHeight': 1, 'fontSize': '1.3em'}),
        html.P("A very rough attempt at a Line-weaver (guessing inhibition type) and dose-response curve",
               style={'textAlign': 'center'}),
        dcc.Graph(
            id='scatter-compound-level',
            style={"height": "40vh", "marginTop": 0, "paddingTop": 0},
        ),
        dcc.Graph(
            id='line-log',
